# Provides rounties for Bayesian Linear regression
# E[y|x_1,...,x_n] = a +Xb where b is the coefficients and X are predictors
# The probability model is given by p[y,a,b,q]=p[y|a,b,q]*p[b|q]*p[a,q] where 1 is the residual precision.
...
def draw_student(loc, scale, df, ndraws):
    ...
    normal_draws = np.random.multivariate_normal(np.zeros(loc.shape[0]), scale, ndraws)
...
def draw_normal_gamma(params, ndraws):
    ...
    gamma_draws =  np.random.gamma(params["shape"], 1/params["rate"],ndraws)

    if type(params["precision"]) != np.ndarray:
        normal_draws = np.array([np.random.normal(params["location"],1/params["precision"]/gamma_draws[i])
...
def integrate(draws, lo, up):
    ...
    up : float or np.ndarray in R^ndi 
    ...
    return np.mean(np.logical_and(lo <= draws, draws <= up))

def get_prediction_interval(draws, alpha = 0.05):
    """ Approximate the (1-alpha)- probability interval from a series of draws. 

    If you pass a series of multivariate draws, the dimension will be scaled 
...
class LinearModel(object):
    """ Computes the posterior distributioln of the models' coefficients. 
    ...
    estiamtes : dict
        point estiamte of the model parameters including "coefficients" and "res_precision"
    """
    ...
    def estimate(self):
        ...
        ## sub_quadrant is the covariance of the prior parameter distribution beta|x,alpha,sigma ~ N(0,g(X'X))

        precision = np.vstack((
            np.hstack((self.nobs,np.sum(self.X,0).T)), # column sum of the element where 0 is equal to axis=0
            np.hstack((np.sum(self.X, 0, keepdims = True).T, sub_quadrant))
        ))
        location = np.linalg.solve(precision, np.dot(design.T, self.y))
        # location = np.linalg.solve(sub_quadrant, np.dot(self.X.T, self.y))

        shape = (self.nobs - 1)/2
        rate = 0.5 * np.dot(self.y - np.dot(design, location), self.y)

        self.posterior = {
            "shape": shape, "rate": rate, "location": location, "precision": precision # instead of precision
        }
        self.estimates = {
            "coefficients": location, "res_precision" : shape / rate
        }

    def predict(self, x_new):
        """ Give point predictions for a new set of obsevations. 
        ...
        return np.dot(design, self.estiamtes["coefficients"])
    ...
    def residual_dist(self, ndraws=1000):
        ...
        return np.random.gamma(
            self.posterior["shape"], 1/self.posterior["rate"], ndraws
        )
