# Liang et al. (2008) Linear model averaging 

import numpy as np 
import bma_sim as core # file of the core.py 
import linear_regression_sim as linear_regression

def log_gamma(x):
    """ Compute the logarithm of the gamma function defined as log Gammma(x) = sum[from i=1 to x-1] log i 
    Parameters
    ----------
    x : int{1,..., inf}
...
class LinearEnumerator(core.Enumerator):
    """ Computes the posteriro probability distribution over the space of linear regression models. 

    This method computes 2^k probabilities, where k is the number of predictors. 
    We use MCMC for larger k. 
    ...
    y : np.ndarray 
        response vector
    posterior : Counter
        posterior distribution over the model space where str(model) is the key 
        and the posterior probability is the value 
    estimates : dict
        point estimates of the model parameters including "coefficients" and "res_precision" 
    """
    ...
    def residual_dist(self,ndraws=1000):
        """ Draw from the posterior distribution of the variance of the residuals. 
    ...
    def _get_likelihood(self,model):
        ...
        - 0.5 * np.log(nobs)
        - (nobs-1)/2 * np.log(np.dot(residuals.T,residuals))
...
class LinearMC3(core.MC3, LinearEnumerator):
    ...
    suitable for high-dimensional models where n<k 
    ...
    y : np.ndarray
        response vector 
    posterior : Counter
        posterior distribution over the model space where str(model) is the key and 
        the posterior probability is the value 
    estimates : dict 
        point estimates of the model parameters including "coefficients" and "res_precision" 
    """
