# Markov Chain Monte Carlo (MCMC) sapling 

import numpy as np 
from collections import Counter

def get_kendalltau_bin(sample1, sample2):
    """ Calculate Kendall's tau-b in 0(n) times. 

    Parameters
    ----------
    sample 1/2 : array_like in {0,1}^ndim 
        binary sequences
...
def est_acf(series):
    ...
    -------
    np.ndarray 
        acf up to lag // 100, starting at lag 1
    """ 
...
def est_int_autocor(acf,tradeoff_par=6):
    ...
    Since there is a bias-variance tradeoff involved in the estimation, the acf is integrated up to lag l 
    such that l is the smallest int for which 
    ...
    A higher parameter lowers the bias and increases the variance. 
...
class MetropolisSampler(object):
    ...
    Draw dependent samples from a probability distribution. 

    Parameters
    ----------
    rv_prob_func : func
        log probability measure on the random variables 
    proposal_func : func
        draw from the proposal distribution given the current state
    proposal_prob_func : func 
        log probaility measure on the proposal distribution 
    ...
    def _run(self, init, niter=100000):
        ...
        # we do draw from the initial state

        for i in range(niter): 
            # repeat with the number of iteration
            candidate ={"draw": self._propose(state["draw"])}
            candidate["prob"] =self._get_rv_prob(candidate["draw"])
            if self._decide(state,candidate):
                state = candidate
            self.draws[i,:] = state["draw"] # matrix of draw from ith row/ while draws[:,i] is draw from ith column 

        # discard burn - in 
        self._diagnose()
        self.draws = self.draws[int(np.max(self.diagnostics["ndiscard"])):,:]

    def _decide(self,state,proposal):
        """ Apply the Metropolis-Hastings decision rule to the candidate state. 
